...
const calcGameStats = (game) => {
  ...
  return {
    ...
    ast: game.assists,
    tov: game.turnovers || 0,
    astToRatio: (game.turnovers || 0) === 0 ? (game.assists > 0 ? "∞" : "—") : (game.assists / game.turnovers).toFixed(1),
    made: made.length,
        ...
        {[
          ...
          <button
            ...
            style={{
              padding: "8px 16px",
              borderRadius: "8px",
              border: view === n.id ? "1.5px solid #D85A30" : "0.5px solid rgba(255,255,255,0.15)",
              background: view === n.id ? "rgba(216,90,48,0.08)" : "#1a1a1a",
              color: view === n.id ? "#993C1D" : "#f0f0f0",
          ...
          {SEASONS.map((s) => (
            <button
              ...
              style={{
                ...
                padding: "8px 4px",
                borderRadius: "8px",
                border: activeSeason === s.id ? `1.5px solid ${s.color}` : "0.5px solid rgba(255,255,255,0.08)",
                background: activeSeason === s.id ? `${s.color}18` : "#242424",
                color: activeSeason === s.id ? s.color : "#9a9a9a",
...
function StatCard({ label, value, sub }) {
  return (
    <div style={{ background: "#242424", borderRadius: "8px", padding: "14px 16px" }}>
      <p style={{ margin: "0 0 4px", fontSize: 12, color: "#9a9a9a", fontWeight: 400 }}>{label}</p>
      <p style={{ margin: 0, fontSize: 24, fontWeight: 500, color: "#f0f0f0" }}>{value}</p>
      {sub && <p style={{ margin: "2px 0 0", fontSize: 11, color: "#666666" }}>{sub}</p>}
    </div>
  );
}
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1rem" }}>
        <h2 style={{ margin: 0, fontSize: 16, fontWeight: 500, color: "#f0f0f0" }}>Season averages</h2>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 10, marginBottom: "1.5rem" }}>
        ...
        <StatCard
          label="AST/TO ratio"
          value={(() => {
            const totalAst = allStats.reduce((sum, s) => sum + s.ast, 0);
            const totalTov = allStats.reduce((sum, s) => sum + s.tov, 0);
            return totalTov === 0 ? (totalAst > 0 ? "∞" : "—") : (totalAst / totalTov).toFixed(1);
          })()}
          sub="season total"
        />
      ...
      <div style={{ background: "#1a1a1a", border: "0.5px solid rgba(255,255,255,0.08)", borderRadius: "12px", padding: "1rem 1.25rem", marginBottom: "1rem" }}>
        <h3 style={{ margin: "0 0 12px", fontSize: 14, fontWeight: 500, color: "#f0f0f0" }}>Shot breakdown per game</h3>
        {SHOT_TYPES.map((st) => {
          ...
          return (
            <div key={st.id} style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
              ...
              <span style={{ fontSize: 12, color: "#9a9a9a", width: 80, textAlign: "right", flexShrink: 0 }}>
    ...
    <div>
      ...
      <div style={{ background: "#242424", borderRadius: "12px", padding: "12px 16px", marginBottom: "1rem", display: "flex", gap: 16, flexWrap: "wrap", alignItems: "center" }}>
        <div style={{ flex: 1, minWidth: 140 }}>
          ...
          <input type="date" value={game.date} onChange={(e) => setGame({ ...game, date: e.target.value })} style={{ fontSize: 13, width: "100%", background: "#1a1a1a", color: "#f0f0f0", border: "0.5px solid rgba(255,255,255,0.15)", borderRadius: 8, padding: "6px 8px" }} />
        </div>
        <div style={{ flex: 2, minWidth: 160 }}>
          <label style={{ fontSize: 11, color: "#9a9a9a", display: "block", marginBottom: 4 }}>Opponent</label>
          <input type="text" value={game.opponent} onChange={(e) => setGame({ ...game, opponent: e.target.value })} placeholder="Opponent name" style={{ fontSize: 13, width: "100%", background: "#1a1a1a", color: "#f0f0f0", border: "0.5px solid rgba(255,255,255,0.15)", borderRadius: 8, padding: "6px 8px" }} />
        </div>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1rem", padding: "12px 16px", background: `${season.color}15`, borderRadius: "12px", border: `0.5px solid ${season.color}40` }}>
        ...
        <div style={{ textAlign: "center" }}>
          ...
          <p style={{ margin: 0, fontSize: 20, fontWeight: 500, color: "#f0f0f0" }}>{s.fgm}/{s.fga}</p>
          <p style={{ margin: 0, fontSize: 11, color: "#9a9a9a" }}>{s.fgPct}%</p>
        </div>
        <div style={{ textAlign: "center" }}>
          <p style={{ margin: 0, fontSize: 11, color: "#9a9a9a" }}>3PT</p>
          <p style={{ margin: 0, fontSize: 20, fontWeight: 500, color: "#f0f0f0" }}>{s.threeM}/{s.threeA}</p>
          <p style={{ margin: 0, fontSize: 11, color: "#9a9a9a" }}>{s.threeA ? Math.round((s.threeM/s.threeA)*100) : 0}%</p>
        </div>
        <div style={{ textAlign: "right" }}>
          <p style={{ margin: 0, fontSize: 11, color: "#9a9a9a" }}>AST/TO</p>
          <p style={{ margin: 0, fontSize: 20, fontWeight: 500, color: "#f0f0f0" }}>{s.astToRatio}</p>
        </div>
      </div>

      {/* Last action toast */}
      {lastAction && (
        <div style={{ textAlign: "center", marginBottom: 8, fontSize: 14, fontWeight: 500, color: lastAction.startsWith("✓") ? "#1D9E75" : "#e24b4a", transition: "opacity 0.3s" }}>
          {lastAction}
        </div>
      )}
        ...
        {SHOT_TYPES.map((st) => {
          ...
          return (
            <div key={st.id} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
              ...
              <span style={{ fontSize: 12, color: "#9a9a9a", width: 60, textAlign: "right", flexShrink: 0 }}>{made}/{att}</span>
            </div>
          );
        })}
      </div>
          ...
          {[
            ...
            <div key={item.label} style={{ background: "#242424", borderRadius: "8px", padding: "12px 14px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
              <div>
                ...
                <p style={{ margin: 0, fontSize: 22, fontWeight: 500, color: "#f0f0f0" }}>{item.val}</p>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                <button onClick={() => incStat(item.key, item.sub)} style={{ width: 30, height: 30, borderRadius: "8px", border: "0.5px solid rgba(255,255,255,0.15)", background: "#1a1a1a", cursor: "pointer", fontSize: 16, fontWeight: 500, display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
                <button onClick={() => decStat(item.key, item.sub)} style={{ width: 30, height: 30, borderRadius: "8px", border: "0.5px solid rgba(255,255,255,0.15)", background: "#1a1a1a", cursor: "pointer", fontSize: 18, display: "flex", alignItems: "center", justifyContent: "center" }}>−</button>
              </div>
            </div>
          ))}
        </div>
...
function HistoryView({ games, season, onDelete, onEdit, editingGame, setEditingGame, onSaveEdit }) {
  ...
  return (
    <div>
      <h2 style={{ margin: "0 0 1rem", fontSize: 16, fontWeight: 500, color: "#f0f0f0" }}>Game log — {games.length} game{games.length !== 1 ? "s" : ""}</h2>
      {sorted.map((g) => {
        const s = calcGameStats(g);
        ...
        return (
          <div key={g.id} style={{ background: "#1a1a1a", border: "0.5px solid rgba(255,255,255,0.08)", borderRadius: "12px", marginBottom: 10, overflow: "hidden" }}>
            ...
            >
              ...
              <i className={`ti ti-chevron-${isOpen ? "up" : "down"}`} style={{ fontSize: 16, color: "#9a9a9a" }} aria-hidden></i>
            </div>
              ...
              <div style={{ borderTop: "0.5px solid rgba(255,255,255,0.08)", padding: "14px 16px" }}>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(100px, 1fr))", gap: 8, marginBottom: 14 }}>
                  {[
                    ...
                    { l: "3PT%", v: `${s.threeA ? Math.round((s.threeM/s.threeA)*100) : 0}%` },
                    { l: "FT", v: `${s.ftm}/${s.fta}` },
                    { l: "Off. reb", v: s.oreb },
                    { l: "Def. reb", v: s.dreb },
                    { l: "Total reb", v: s.reb },
                ...
                <div style={{ marginBottom: 14 }}>
                  <p style={{ margin: "0 0 8px", fontSize: 12, color: "#9a9a9a" }}>Shot breakdown</p>
                  {SHOT_TYPES.map((st) => {
                    ...
                    return (
                      <div key={st.id} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                        ...
                        <span style={{ fontSize: 11, color: "#9a9a9a", width: 70, textAlign: "right" }}>{made}/{att} ({pct}%)</span>
                      </div>
                    );
                  })}
                </div>
                ...
                {g.notes && (
                  <p style={{ margin: "0 0 14px", fontSize: 13, color: "#9a9a9a", fontStyle: "italic" }}>{g.notes}</p>
                )}
